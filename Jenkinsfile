    ...
    post {
        ...
        stage ('singet tap commands execution ') {
            steps {
                ...
                docker exec tap tap-ordway -c /app/ordway-tap/config.json --catalog /app/ordway-tap/catalog.json  | /usr/local/bin/target-stitch  --config /app/ordway-tap/stitch_config.json
                """
                }
            post {
                success {
